...
1. **Isolated context** — the subagent runs in its own blank context window. It doesn't see the parent's history.
2. **Outbound** — the parent passes it *just a prompt*: the reworded task. Nothing else.
3. **Inbound** — only the *final result* comes back, as a tool result in the parent's loop. The subagent's intermediate steps don't pollute it.

<Callout tone="info">
  It's still the harness's `call → observe → decide → repeat` loop — except here one of the "tools" is "run another agent." That's the whole magic of inter-agent communication.
</Callout>
