...
```
   assistant-ui (local chat)
        │  useLangGraphRuntime  ←→  langgraph dev (local server)
        ▼
   Deep agent (dispatcher) ──► classifies the request, delegates
        │        (write_todos if multi-step)
        ├─ "how much on dining?"    ─► chat       (answers directly)
        ├─ "pull my subscriptions"  ─► extractor  (→ memory/extraction-06.md)
        └─ "June's report"          ─► reporter   (reads extraction → report + optims)
        │            ▲
        │            └─ extract_statement(2026-06.pdf) ─► text, extracted locally
        ▼
   filesystem   statements/2026-06.pdf        (raw statement, out of context)
                memory/extraction-06.md        (shared workspace across agents)
                memory/budget.md               (memory, persistent)
                memory/report-06.md            (deliverable)
        │
        ▼
   Answer / report streamed into the chat
```
