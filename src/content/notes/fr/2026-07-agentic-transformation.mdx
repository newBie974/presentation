...
<Callout type="note">
**Le décalage mental.** Avant, une erreur de configuration = l'agent crash, le service down, il faut redéployer, tout le monde attend. Maintenant c'est : « erreur de config = on la loggue, on la rejette, on continue avec la version précédente ». Le système devrait fonctionner même quand les gens font des erreurs. Surtout quand les gens font des erreurs. C'est résilience par design, pas par chance.
</Callout>
