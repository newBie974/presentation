...
```
   assistant-ui (chat local)
        │  useLangGraphRuntime  ←→  langgraph dev (serveur local)
        ▼
   Deep agent (dispatcher) ──► classe la demande, délègue
        │        (write_todos si multi-étapes)
        ├─ "combien en resto ?"      ─► chat        (répond direct)
        ├─ "sors mes abonnements"    ─► extracteur  (→ memoire/extraction-06.md)
        └─ "le rapport de juin"      ─► rapporteur  (lit l'extraction → rapport + optims)
        │            ▲
        │            └─ extraire_releve(2026-06.pdf) ─► texte, extrait en local
        ▼
   filesystem   releves/2026-06.pdf          (relevé brut, hors contexte)
                memoire/extraction-06.md      (atelier partagé entre agents)
                memoire/budget.md             (mémoire, persistante)
                memoire/rapport-06.md         (livrable)
        │
        ▼
   Réponse / rapport streamé dans le chat
```
