export const SUPPORTED_LOCALES = ["fr", "en"] as const;
export type Locale = (typeof SUPPORTED_LOCALES)[number];
export const DEFAULT_LOCALE: Locale = "fr";

export const ui = {
  fr: {
    "nav.home": "Accueil",
    "nav.about": "À propos",
    "nav.notes": "Notes",
    "nav.work": "Collaborer",
    "nav.ariaPrimary": "Navigation principale",
    "a11y.language": "Langue",
    "cta.viewApps": "Voir mes apps",
    "cta.collaborate": "Collaborer",
    "cta.bookCall": "Réserver un call",
    "cta.readArticle": "Lire l'article",
    "cta.readMore": "Lire l'histoire complète",
    "cta.visitSite": "Voir le site",
    "cta.demo": "Démo",
    "cta.code": "Code",
    "cta.viewAllLab": "Voir tout le lab",
    "section.lab": "Le lab",
    "section.builds": "Ce que je construis",
    "filter.legend": "Filtrer par type",
    "filter.all": "Tout",
    "filter.app": "App",
    "filter.web": "Web",
    "section.latestNotes": "Dernières notes",
    "section.allNotes": "Toutes les notes",
    "section.about": "À propos",
    "social.follow": "Suivre",
    "status.live": "Live",
    "status.building": "En cours",
    "status.archived": "Arrêté",
    "apps.downloadOn": "Télécharger sur",
    "apps.getItOn": "Disponible sur",
    "showcase.inProduction": "En production",
    "showcase.discover": "Découvrir l'app",
    "showcase.pageTitleLead": "Mes ",
    "showcase.pageTitleAccent": "réalisations",
    "showcase.pageDeck":
      "Quatre apps en production, conçues, développées et publiées en solo. Voici à quoi elles ressemblent vraiment.",
    "showcase.seeAll": "Voir mes réalisations",
    "offer.label": "L'OFFRE",
    "offer.heading": "Produit sur mesure",
    "offer.featured": "Le plus demandé",
    "offer.supportLabel": "ACCOMPAGNEMENT",
    "offer.supportHeading":
      "Quand tu as besoin d'un renfort, pas d'un projet complet",
    "offer.trustLabel": "TA DÉCISION, DÉRISQUÉE",
    "app.features": "Fonctionnalités clés",
    "app.forWho": "Pour qui",
    "app.about": "L'app en bref",
    "app.backHome": "← Toutes les apps",
    "status.dispoLabel": "Dispo S2 2026 · 1 slot",
    "footer.copy": "© 2026 Aymeric Dijoux · La Réunion & Paris",
    "pagination.prev": "Précédent",
    "pagination.next": "Suivant",
    "pagination.page": "Page",
    "pagination.label": "Pagination",
    "notes.sectionLabel": "NOTES",
    "notes.heroPre": "Ce que ",
    "notes.heroHl": "j'apprends",
    "notes.heroPost": " en buildant.",
    "notes.deck":
      "Notes de build, retours d'expérience, et essais techniques d'Aymeric Dijoux. Pas un blog régulier — j'écris quand j'ai quelque chose qui vaut le coup d'être partagé.",
    "notes.basedIn": "Basé à Paris · Épinay-sur-Orge (Essonne) & La Réunion",
    "notes.metaTitle": "Notes — Aymeric Dijoux",
    "notes.featured": "À la une",
    "notes.allTags": "tous les tags",
    "notes.filtersLabel": "Filtres tags",
    "notes.allPill": "tous",
    "notes.rss": "RSS",
    "notes.empty": "Pas encore d'article — reviens bientôt.",
    "note.readNext": "Lire ensuite",
    "note.ctaLead": "Un truc à builder ensemble ?",
    "tags.sectionLabel": "TAGS",
    "tags.title": "Tags",
    "tags.deck": "Tous les sujets abordés dans les notes.",
    "tags.metaTitle": "Tags — Aymeric Dijoux",
    "tags.back": "← Toutes les notes",
  },
  en: {
    "nav.home": "Home",
    "nav.about": "About",
    "nav.notes": "Writing",
    "nav.work": "Work together",
    "nav.ariaPrimary": "Primary navigation",
    "a11y.language": "Language",
    "cta.viewApps": "See my apps",
    "cta.collaborate": "Work together",
    "cta.bookCall": "Book a call",
    "cta.readArticle": "Read the article",
    "cta.readMore": "Read the full story",
    "cta.visitSite": "Visit site",
    "cta.demo": "Demo",
    "cta.code": "Code",
    "cta.viewAllLab": "See the whole lab",
    "section.lab": "The lab",
    "section.builds": "What I'm building",
    "filter.legend": "Filter by type",
    "filter.all": "All",
    "filter.app": "App",
    "filter.web": "Web",
    "section.latestNotes": "Latest writing",
    "section.allNotes": "All writing",
    "section.about": "About",
    "social.follow": "Follow",
    "status.live": "Live",
    "status.building": "Building",
    "status.archived": "Archived",
    "apps.downloadOn": "Download on",
    "apps.getItOn": "Get it on",
    "showcase.inProduction": "In production",
    "showcase.discover": "See the app",
    "showcase.pageTitleLead": "What I've ",
    "showcase.pageTitleAccent": "shipped",
    "showcase.pageDeck":
      "Four apps in production, designed, built and shipped solo. Here is what they actually look like.",
    "showcase.seeAll": "See everything I've shipped",
    "offer.label": "THE OFFER",
    "offer.heading": "Custom product",
    "offer.featured": "Most requested",
    "offer.supportLabel": "SUPPORT",
    "offer.supportHeading": "When you need reinforcement, not a full project",
    "offer.trustLabel": "YOUR DECISION, DE-RISKED",
    "app.features": "Key features",
    "app.forWho": "For whom",
    "app.about": "The app in short",
    "app.backHome": "← All apps",
    "status.dispoLabel": "Available H2 2026 · 1 slot",
    "footer.copy": "© 2026 Aymeric Dijoux · La Réunion & Paris",
    "pagination.prev": "Previous",
    "pagination.next": "Next",
    "pagination.page": "Page",
    "pagination.label": "Pagination",
    "notes.sectionLabel": "WRITING",
    "notes.heroPre": "What I'm ",
    "notes.heroHl": "learning",
    "notes.heroPost": " while building.",
    "notes.deck":
      "Build notes, lessons learned, and technical essays by Aymeric Dijoux. Not a regular blog — I write when I have something worth sharing.",
    "notes.basedIn":
      "Based in Paris · Épinay-sur-Orge (Essonne) & Réunion Island",
    "notes.metaTitle": "Writing — Aymeric Dijoux",
    "notes.featured": "Featured",
    "notes.allTags": "all tags",
    "notes.filtersLabel": "Tag filters",
    "notes.allPill": "all",
    "notes.rss": "RSS",
    "notes.empty": "No articles yet — check back soon.",
    "note.readNext": "Read next",
    "note.ctaLead": "Something to build together?",
    "tags.sectionLabel": "TAGS",
    "tags.title": "Tags",
    "tags.deck": "Every topic covered in the notes.",
    "tags.metaTitle": "Tags — Aymeric Dijoux",
    "tags.back": "← All writing",
  },
} as const satisfies Record<Locale, Record<string, string>>;

export type UiKey = keyof (typeof ui)["fr"];

export const routes = {
  fr: {
    home: "/",
    about: "/a-propos",
    notes: "/notes",
    work: "/collaborer",
    now: "/now",
    lab: "/lab",
    apps: "/apps",
    tags: "/notes/tags",
    rss: "/rss.xml",
  },
  en: {
    home: "/en/",
    about: "/en/about",
    notes: "/en/writing",
    work: "/en/work",
    now: "/en/now",
    lab: "/en/lab",
    apps: "/en/apps",
    tags: "/en/writing/tags",
    rss: "/en/rss.xml",
  },
} as const;
